---
name: open-canvas
description: 打开映雪网页画布并自动连接本地 Canvas Agent。用户要求打开、启动、进入、使用映雪或画布时使用。
---

# 打开映雪画布

当用户要求打开、启动、进入或使用映雪时，不要把 URL 交给用户手动复制，不要通过浏览器点击“新建画布”。优先快速拉起本地画布和本地 Canvas Agent，然后直接打开带 `mode`、`agentUrl`、`agentToken` 的 URL，让网页自动创建或选择画布并连接 Agent。
...
1. 如果当前仓库是映雪项目，优先使用当前仓库的 `web/` 前端。
...
- 如果当前工作区不是映雪源码仓库，优先提示用户先打开或启动映雪网页，再连接本地 Agent。
