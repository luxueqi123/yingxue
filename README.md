...
当前代码已同步上游 `v1.2.3.1` 能力，仍在快速迭代。数据结构和外部接口可能随产品升级调整，建议先在个人、本地或可信环境中使用。
...
生产环境只需公开 Web 端口；后端 `8080` 应保留在 Compose 网络内。部署前请根据 `.env.example` 配置 HTTPS、跨域来源、数据目录和注册开关。内置技能的案例图片、视频和封面通过 `CANVAS_SKILL_MEDIA_PATH` 从宿主机数据盘只读挂载，不应写入容器可写层。

### 上游在线更新机制

上游 `v1.2.3` 起提供“系统配置 → 系统更新”。后台页面只负责管理员鉴权、状态展示和发起操作；真正的镜像拉取、PostgreSQL 与数据目录备份、迁移、健康检查和失败回退由宿主机 `open-ai-canvas-host-updater` 通过带 Token 的 Unix Socket 执行，业务 Backend 不直接挂载 Docker Socket。完整流程见[在线更新与安全回退](docs/content/docs/backend/system-update.mdx)。

映雪包含独立品牌、渠道、计费、登录页和插件定制，不能直接用上游官方 GHCR 镜像在线覆盖，否则会退回上游原版。启用生产在线更新前，必须先建立映雪自己的固定版本镜像、Release 资产与回退链路。
...
- PostgreSQL、Redis、上传文件、对象存储、技能案例媒体和备份都应使用持久化数据目录，并限制访问权限。
