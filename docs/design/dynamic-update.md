...
> 状态说明：本文记录完整的后续演进设计，不代表所有条目已经上线。当前可用实现、安装命令和安全回退合同以 `docs/content/docs/backend/system-update.mdx` 为准；签名 `update-manifest.json`、稳定版通道和超级管理员分级仍未实现，不得作为现役能力对外承诺。
...
第一阶段后台提供：当前版本、目标版本、发布时间、更新日志、通道、检查时间、升级前检查结果、更新进度、失败原因、回退结果和审计记录。现役账号模型只有 `admin` 与普通用户，更新 API 由服务端要求 `admin`；若未来新增超级管理员分级，必须先更新权限模型、接口测试和本设计。危险状态不提供“强制跳过迁移/备份/验签”入口。
