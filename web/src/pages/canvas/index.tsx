...
export default function CanvasPage() {
    ...
    useEffect(() => {
        if (!hydrated || !sessionHydrated || (userId && !libraryQuery.isSuccess) || autoOpenRef.current || (mode !== "new" && mode !== "recent" && mode !== "handoff" && mode !== "prompt")) return;
    ...
    if (hydrated && !libraryQuery.isError && (mode === "new" || mode === "recent" || mode === "handoff" || mode === "prompt")) return <main className="flex h-full items-center justify-center bg-background text-sm text-stone-500">正在打开画布...</main>;
