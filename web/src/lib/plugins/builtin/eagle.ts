...
Eagle 插件把桌面 Eagle 资料库接入映雪素材面板。浏览器不会直接访问 Eagle：所有读取、搜索、下载和写入请求先到映雪后端，再由后端访问管理员配置的 Eagle Local API 地址。
...
> Eagle 必须保持运行。关闭客户端、切换资料库或修改本地 API 端口后，映雪会收到连接失败，不能用空列表掩盖。
...
导入时，后端代理下载 Eagle 原文件，前端再写入映雪自己的图片或媒体存储。图片保留可识别的宽高和 MIME；视频、音频和其他模型文件保留字节数、扩展名、Eagle item ID、文件夹和来源元数据。
...
插件可把映雪图片、视频或音频转换为 data URL，再调用 Eagle 添加素材。已有本地 Blob 优先直接读取；只有没有本地副本时才下载远程生成地址。默认单个手动上传文件不超过 96 MB，且 MIME 必须以 \`image/\`、\`video/\` 或 \`audio/\` 开头。

启用“自动上传生成结果”后，生成资产可以写入配置的目标文件夹。自动写回失败必须显示为写回失败，不应把“映雪已保存”误报为“Eagle 已保存”。
